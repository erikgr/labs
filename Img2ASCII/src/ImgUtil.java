...
public class ImgUtil {
	...
	public static BufferedImage rescale_nn (BufferedImage input, int dst_width, int dst_height) {
		...
		if((input.getWidth() == dst_width) && (input.getHeight() == dst_height)){ return output; }
	...
	public static char[][] to_ascii (BufferedImage img) throws Exception {
		...
		//char[] grayscale = new char[] { '$', '@', 'B', '%', '8', '&', 'W', 'M', '#', '*', 'o', 'a', 'h', 'k', 'b', 'd', 'p', 'q', 'w', 'm', 'Z', 'O', '0', 'Q', 'L', 'C', 'J', 'U', 'Y', 'X', 'z', 'c', 'v', 'u', 'n', 'x', 'r', 'j', 'f', 't', '/', '\\', '|', '(', ')', '1', '{', '}', '[', ']', '?', '-', '_', '+', '~', '<', '>', 'i', '!', 'l', 'I', ';', ':', ',', '"', '^', '`', '\'', '.', ' '};
		int height			= img.getHeight();
		int width			= img.getWidth();
		int mask			= 255;
	...
	public static int[][] to_ANSIVT100_colormap (BufferedImage img) throws Exception {
		...
		for(int y=0; y<height; y++){
			for(int x=0; x<width; x++){
				int r = ImgUtil.getR(img, x, y);
				int g = ImgUtil.getG(img, x, y);
				int b = ImgUtil.getB(img, x, y);
				int color_code = TerminalUtil.rgb_to_ANSI_color(r, g, b, 255); // assuming 8bit depth --> 255
				output[x][y] = color_code;
			}
		}
	...
	public static void print_image_ANSIVT100_colors (BufferedImage img, int dst_width) throws Exception {
		int width = img.getWidth();
		int height = img.getHeight();
		double rsz_factor = (double) dst_width / (double) width;
		int dst_height = (int) (rsz_factor * height);
		BufferedImage rs = ImgUtil.rescale_nn(img, dst_width, dst_height);
		char[][] ascii = ImgUtil.to_ascii(ImgUtil.to_grayscale(rs));
		int[][] color_map = ImgUtil.to_ANSIVT100_colormap(rs);
		
		for(int y=0; y<dst_height; y++){
			for(int x=0; x<dst_width; x++){
				System.out.print(TerminalUtil.print_BG(color_map[x][y]) + " ");
			}
			System.out.println(TerminalUtil.remove_ANSI_attributes());
		}
